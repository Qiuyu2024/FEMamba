import os #用于与操作系统交互，通常用于文件路径、环境变量等操作。
import torch
...
def import_abspy(name="models", path="classification/"): #动态地从指定路径导入一个 Python 模块
    import sys
    import importlib
...
def print_jit_input_names(inputs): #可能用于调试或输出信息，尤其是在处理一些如 JIT（Just-In-Time）编译的框架时，
    print("input params: ", end=" ", flush=True)
    try:
        for i in range(10):
            print(inputs[i].debugName(), end=" ", flush=True) #调试错误代码时候使用
...
def flops_selective_scan_ref(B=1, L=256, D=768, N=16, with_D=True, with_Z=False, with_Group=True, with_complex=False):
    """ #计算并返回一个深度学习模型（或类似计算图）的 FLOPs（浮点运算量）
    u: r(B D L)
    delta: r(B D L)
    A: r(D N)
    B: r(B N L)
