...
class frequency_feature(nn.Module): #(FFEM)
...
class Local_feature(nn.Module): #LSAM
    """
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        self.fft_branch0 = frequency_feature(in_channel=hidden_dim)
        self.drop_path = DropPath(drop_path)
        self.skip_scale= nn.Parameter(torch.ones(hidden_dim))
        self.fft_branch = frequency_feature(in_channel=hidden_dim)
        self.localblock = Local_feature(dim=hidden_dim,hidden_dim=int(2*hidden_dim))
    ...
    def forward(self, input, x_size):
        ...
        x = x * self.skip_scale2 + fft+ self.localblock(self.ln_2(x).permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3,1).contiguous()  # B H W C
...
class MambaIR(nn.Module):
    ...
    def forward(self, x):
        self.mean = self.mean.type_as(x) #对输入 x 进行归一化处理，将输入调整到某个数值范围中，以便后续网络能够更高效地处理特征
        #self.mean 的数据类型转换为与 x 相同的数据类型
        #self.mean 通常是图像通道的平均值，比如 RGB 图像的均值可能是 [0.485, 0.456, 0.406]具体数字存疑
